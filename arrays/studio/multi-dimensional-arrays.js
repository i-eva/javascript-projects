...
cabNum = input.question("Which cabinet would you like to look in for a particular item? ");
let item = input.question("What item are you looking for? ");

if (cabNum < cargoHold.length && cabNum >= 0) {
    if (cargoHold[cabNum].includes(item)) {}
    console.log(`The contents of cabinet ${cabNum} DOES contain: ${item}`);
} else {
    console.log(`Cabinet ${cabNum} DOES NOT contain ${uten}.`)
}
